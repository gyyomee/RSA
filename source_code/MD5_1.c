...
/* 함수선언 */
void padding(BYTE* in, UINT64 msg_len); //MD-5 패딩함수
void MD5_init(); //MD-5 초기화함수
void MD5(FILE* fptr, BYTE* result); //MD-5 해쉬함수
void MD5_digest(BYTE *in); //MD-5 메시지 digest함수
...
void make_Bit128(BYTE in[16], UINT a, UINT b, UINT c, UINT d); //word단위의 해쉬 값을 byte단위로 변환하는 함수
