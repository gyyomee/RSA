...
void put_to_message(unsigned char* msg, short *a, short mn); //이진 형태의 메시지를 버퍼에 저장하는 함수
...
short ps[LEN_PS * 8]; //패딩 스트링
					  ...
					  //암호와 복호에 사용되는 버퍼(Radix와 octet형태)
INT64 S[mb]; //암호문
...
void RSA_Enc(unsigned char* p_text, unsigned char* result)
{
	...
	while (check == 1)
	{
		...
		if (check != -1)
		{
			...
			/*** c = m^e mod n (m-bit) ***/
			LeftTORight_Pow(EB1, E, S, N, mb); //수신자의 공개 키로 암호화

		 //Radix 형태의 암호문을 이진 형태로 반환
			CONV_R_to_B(S, s, mb);
...
void RSA_Dec(unsigned char* c_text, unsigned char* result)
{
	...
	while (check == 1)
	{
		...
		if (check != -1)
		{
			...
			CONV_R_to_B(H, v_h, mb); //복호화된 데이터를 이진 형태로 반환
			CONV_B_to_O(v_h, D_EB, mb * 4); //이진형태의 데이터를 octet으로 반환
